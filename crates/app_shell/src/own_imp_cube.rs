...
fn draw_cube_interactive(
    ...
    // Build geometry by computing exact vertex positions
    // Each vertex is identified by which corner it's near and which face it belongs to

    // For a chamfered cube, vertices on a main face are pulled inward from the corner
    // The amount of pull depends on the chamfer value m

    // Vertex naming: v_{corner}_{face}
    // corner: ppp = (+,+,+), ppn = (+,+,-), etc where p=positive, n=negative
    // face: X, Y, Z for the face normal direction (positive or negative)

    // Helper: compute a vertex at a corner, pulled toward a specific face
    // corner: the corner position (each component is +1 or -1)
    // face_axis: 0=X, 1=Y, 2=Z - which axis the face normal is along
    // The vertex is at the corner but pulled inward along the two non-face axes
    let vertex = |cx: f32, cy: f32, cz: f32, face_axis: usize| -> Vec3 {
        let mut v = Vec3::new(cx, cy, cz);
        // Pull inward along the two axes that are NOT the face axis
        for axis in 0..3 {
            if axis != face_axis {
                // Pull toward center by m * 2 (to match FreeCAD's x2/y2 formula)
                v[axis] *= 1.0 - m * 2.0;
            }
        }
        v
    ...
    // Each main face has 8 vertices - 2 per corner of that face

    // Front face (+Z) - vertices at z=1
    polygons.push(CubePolygon {
        verts: vec![
            vertex(-1.0, -1.0, 1.0, 2), // bottom-left, from Y edge
            vertex(-1.0, -1.0, 1.0, 0), // bottom-left, from X edge
            vertex(1.0, -1.0, 1.0, 0),  // bottom-right, from X edge
            vertex(1.0, -1.0, 1.0, 2),  // bottom-right, from Y edge
            vertex(1.0, 1.0, 1.0, 2),   // top-right, from Y edge
            vertex(1.0, 1.0, 1.0, 0),   // top-right, from X edge
            vertex(-1.0, 1.0, 1.0, 0),  // top-left, from X edge
            vertex(-1.0, 1.0, 1.0, 2),  // top-left, from Y edge
        ],
        normal: Vec3::Z,
        color: front_color,
        label: Some("FRONT"),
    ...
    // Each edge connects two main faces

    // Front-Top edge (z=1, y=1)
    polygons.push(CubePolygon {
        verts: vec![
            vertex(-1.0, 1.0, 1.0, 2), // front face
            vertex(1.0, 1.0, 1.0, 2),  // front face
            vertex(1.0, 1.0, 1.0, 1),  // top face
            vertex(-1.0, 1.0, 1.0, 1), // top face
        ],
    ...
    // Front-Bottom edge (z=1, y=-1)
    polygons.push(CubePolygon {
        verts: vec![
            vertex(-1.0, -1.0, 1.0, 1), // bottom face
            vertex(1.0, -1.0, 1.0, 1),  // bottom face
            vertex(1.0, -1.0, 1.0, 2),  // front face
            vertex(-1.0, -1.0, 1.0, 2), // front face
        ],
    ...
    // Front-Right edge (z=1, x=1)
    polygons.push(CubePolygon {
        verts: vec![
            vertex(1.0, -1.0, 1.0, 2), // front face
            vertex(1.0, 1.0, 1.0, 2),  // front face
            vertex(1.0, 1.0, 1.0, 0),  // right face
            vertex(1.0, -1.0, 1.0, 0), // right face
        ],
    ...
    // Front-Left edge (z=1, x=-1)
    polygons.push(CubePolygon {
        verts: vec![
            vertex(-1.0, -1.0, 1.0, 0), // left face
            vertex(-1.0, 1.0, 1.0, 0),  // left face
            vertex(-1.0, 1.0, 1.0, 2),  // front face
            vertex(-1.0, -1.0, 1.0, 2), // front face
        ],
    ...
    // Rear-Top edge (z=-1, y=1)
    polygons.push(CubePolygon {
        verts: vec![
            vertex(1.0, 1.0, -1.0, 2),  // rear face
            vertex(-1.0, 1.0, -1.0, 2), // rear face
            vertex(-1.0, 1.0, -1.0, 1), // top face
            vertex(1.0, 1.0, -1.0, 1),  // top face
        ],
    ...
    // ===== CORNER BEVELS (8 triangles) =====
    // Each corner connects three faces - it's a triangle using one vertex from each face

    // Front-Top-Right corner (+X, +Y, +Z)
    polygons.push(CubePolygon {
        verts: vec![
            vertex(1.0, 1.0, 1.0, 0), // right face vertex
            vertex(1.0, 1.0, 1.0, 1), // top face vertex
            vertex(1.0, 1.0, 1.0, 2), // front face vertex
        ],
    ...
    // Front-Top-Left corner (-X, +Y, +Z)
    polygons.push(CubePolygon {
        verts: vec![
            vertex(-1.0, 1.0, 1.0, 2), // front face vertex
            vertex(-1.0, 1.0, 1.0, 1), // top face vertex
            vertex(-1.0, 1.0, 1.0, 0), // left face vertex
        ],
    ...
    // Front-Bottom-Right corner (+X, -Y, +Z)
    polygons.push(CubePolygon {
        verts: vec![
            vertex(1.0, -1.0, 1.0, 2), // front face vertex
            vertex(1.0, -1.0, 1.0, 1), // bottom face vertex
            vertex(1.0, -1.0, 1.0, 0), // right face vertex
        ],
    ...
    // Front-Bottom-Left corner (-X, -Y, +Z)
    polygons.push(CubePolygon {
        verts: vec![
            vertex(-1.0, -1.0, 1.0, 0), // left face vertex
            vertex(-1.0, -1.0, 1.0, 1), // bottom face vertex
    ...
    // Rear-Top-Right corner (+X, +Y, -Z)
    polygons.push(CubePolygon {
        verts: vec![
            vertex(1.0, 1.0, -1.0, 2), // rear face vertex
            vertex(1.0, 1.0, -1.0, 1), // top face vertex
            vertex(1.0, 1.0, -1.0, 0), // right face vertex
        ],
    ...
    // Rear-Bottom-Right corner (+X, -Y, -Z)
    polygons.push(CubePolygon {
        verts: vec![
            vertex(1.0, -1.0, -1.0, 0), // right face vertex
            vertex(1.0, -1.0, -1.0, 1), // bottom face vertex
